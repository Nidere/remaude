// Юнит-проверка markdown-рендерера: разметка + защита от html-инъекций.
...
const cases = [
  ...
  ['[зло](javascript:alert(1))', []], // не должен стать ссылкой
...
for (const [src, expects] of cases) {
  ...
  if (src.includes('javascript:') && html.includes('<a ')) {
    console.error(`FAIL: js-ссылка прошла: ${html}`);
    failed++;
  }
  if (src.includes('<script>') && html.includes('<script>')) {
    console.error(`FAIL: html-инъекция: ${html}`);
    failed++;
  }
}
